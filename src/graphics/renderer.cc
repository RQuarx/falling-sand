...
auto
renderer::get_layout() const noexcept -> grid_layout
{ return m_layout; }
...
renderer::render_grid(gfx::context &ctx, const core::grid &grid, ui::cursor &cursor)
{
    ...
    if (sdl::size window_size { ctx.window().get_size_in_pixels() }; window_size != m_window_size)
    {
        ...
        m_signal_on_size_changed.emit(grid.size(), grid_layout { m_layout });
    }
...
{
    m_layout.grid_rect = calculate_grid_rect(m_window_size, grid_size);
    m_layout.cell_size = calculate_cell_size(m_layout.grid_rect, grid_size);

    if ((m_grid_texture.get() == nullptr) || m_grid_texture.size() != grid_size)
    {
        m_grid_texture.reset(renderer.create_texture(SDL_PIXELFORMAT_RGBA8888,
                                                     SDL_TEXTUREACCESS_STREAMING, grid_size));
